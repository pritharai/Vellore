...
//   const handleAddToWishlist = (variantId, size) => {
//     if (!isAuthenticated) {
//       toast.error("Please log in to add to wishlist");
//       setTimeout(() => navigate("/auth"), 2000);
//       return;
...
//   if (isLoading) return <div className="text-center py-12">Loading cart...</div>;
//   if (error) return <div className="text-center py-12 text-red-500">Failed to load cart</div>;
//   if (!cart?.items?.length) return <div className="text-center py-12">Your cart is empty</div>;
...
//                 <div className="inline-flex border border-gray-300 rounded-md overflow-hidden">
//                   <button
//                     onClick={() => startEditing(item._id, item.quantity)}
//                     className="px-3 py-1 font-bold hover:cursor-pointer"
...
//               </button>
//               <button
//                 onClick={() => handleAddToWishlist(item.variant._id, item.size)}
...
//             </button>
//             <button
//               className="ml-4 px-6 py-3 bg-primary text-white rounded-md hover:bg-primary-hover transition hover:cursor-pointer"
//             >
//               Buy Now
//             </button>
//           </div>
...
const Cart = () => {
  ...
  // Wishlist mutation
  const wishlistMutation = useMutation({
    mutationFn: ({ variantId, size }) => addToWishlist({ variantId, size }), // Only pass variantId and size to addToWishlist
  ...
  const handleAddToWishlist = (variantId, size, cartItemId) => {
    ...
    wishlistMutation.mutate({
      variantId,
      size,
      cartItemId, // Include cartItemId for tracking
    });
  };
  ...
  if (error) return <div className="text-center py-12 text-red-500 max-w-[1100px] md:mt-50 mx-auto p-6 mb-20">Failed to load cart</div>;
  if (!cart?.items?.length) return <div className="max-w-[1100px] md:mt-50 mx-auto p-6 mb-20">
    <h2 className="text-2xl font-semibold mb-6">Your Cart</h2>

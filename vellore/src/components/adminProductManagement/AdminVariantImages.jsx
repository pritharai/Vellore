...
const AdminVariantImages = () => {
  ...
  const [variantImagesModal, setVariantImagesModal] = useState({
    open: false,
    mode: "create",
    data: null,
  });
  ...
  const {
    ...
    queryKey: ["variants", variantPage],
    queryFn: () => getVariants({ limit, page: variantPage }),
  });
  ...
  const openVariantImagesModal = async (variantId) => {
    try {
      ...
      if (variantImages?.images?.length > 0) {
        setVariantImagesModal({
          open: true,
          mode: "update",
          data: { variantId, _id: variantImages._id },
        });
      } else {
        setVariantImagesModal({
          open: true,
            ...
            <table className="w-full border-collapse text-xs md:text-sm min-w-full">
              <thead>
                <tr className="bg-gray-100 text-gray-700">
                  <th className="border-b p-1.5 md:p-3 text-left min-w-[150px]">
                    Variant
                  </th>
                  <th className="border-b p-1.5 md:p-3 text-left min-w-[100px]">
                    Images Count
                  </th>
                  <th className="border-b p-1.5 md:p-3 text-center min-w-[200px]">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody>
                {variantsData.variants.map((variant, index) => (
                  ...
                  >
                    <td className="border-b p-1.5 md:p-3">
                      {variant.product.name} - {variant.color.name}
                    </td>
                    <td className="border-b p-1.5 md:p-3">
                      {imageQueries[index]?.data ?? 0}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 md:p-6 w-full max-w-[90vw] md:max-w-md">
            <h2 className="text-lg md:text-xl font-bold mb-4">
              {variantImagesModal.mode === "create"
                ? "Add Variant Images"
                : "Edit Variant Images"}
            </h2>
            <form
              onSubmit={variantImagesForm.handleSubmit((data) => {
                ...
                if (variantImagesModal.mode === "create") {
                  ...
                  updateVariantImagesMutation.mutate({
                    id: variantImagesModal.data._id,
                    data: formData,
                  });
                }
              })}
            >
              <div className="mb-4">
                ...
                <input
                  type="text"
                  value={
                    variantsData?.variants?.find(
                      (v) => v._id === variantImagesModal.data?.variantId
                    )?.product.name || ""
                  }
                  disabled
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  accept="image/*"
                  multiple
                  {...variantImagesForm.register("images", {
                ...
                {variantImagesForm.formState.errors.images && (
                  <p className="text-red-500 text-xs">
                    {variantImagesForm.formState.errors.images.message}
                  </p>
                )}
              </div>
              <div className="flex flex-col lg:flex-row justify-between gap-2">
                <button
                  ...
                  onClick={async () => {
                    const images = await getVariantImages({
                      variantId: variantImagesModal.data?.variantId,
                    });
                    if (images?.images?.length) {
                      handleDelete("variantImages", images._id);
                    }
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={
                      createVariantImagesMutation.isPending ||
                      updateVariantImagesMutation.isPending
                    }
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 md:p-6 w-full max-w-[90vw] md:max-w-md">
            ...
            <div className="flex justify-end mt-4">
              <button
                onClick={() =>
                  setViewImagesModal({ open: false, variantId: null })
                }
                className="bg-gray-300 text-gray-700 px-2 md:px-3 py-1 md:py-1.5 rounded hover:bg-gray-400 transition text-xs md:text-sm"
              >
                Close
...
const VariantImages = ({ variantId }) => {
  ...
  if (isLoading)
    return (
      <p className="text-gray-500 text-sm md:text-base">Loading images...</p>
    );
  if (error)
    return (
  ...
  if (!variantImagesData?.images?.length)
    return (
      <p className="text-gray-500 text-sm md:text-base">No images uploaded</p>
    );

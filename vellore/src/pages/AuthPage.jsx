...
// Kept UI the same, but simplified logic as requested.
// Removed commented-out useEffect (handle redirect in PrivateRoute instead).
// Using useAuth hook for login/register.
// Form validation remains simple.
...
const AuthPage = () => {
  const [isLogin, setIsLogin] = useState(true);
  const navigate = useNavigate();
  const { login, register, isLoggingIn, error } = useAuth();
        ...
        >
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={isLoggingIn}
              className="w-full bg-primary text-white py-3 rounded-full hover:cursor-pointer hover:bg-primary-hover transition duration-300 disabled:opacity-50"
            >
              {isLoggingIn ? "Processing..." : isLogin ? "Login" : "Sign Up"}
            </button>
          </form>
